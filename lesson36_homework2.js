...
function getStringProgran(schedule) {
    var result = schedule.filter(function (el) { return el._embedded.show.schedule.time === ""; });
    return result.map(function (el) { return el._embedded.show.name; }).join();
}
console.log(getStringProgran(schedule));
function getGenresShow(schedule) {
    var result = schedule.filter(function (el) { return el._embedded.show.genres.length > 1; });
    return result.map(function (el) { return el.name + ":[" + el._embedded.show.genres + "]"; });
}
console.log(getGenresShow(schedule));
function getTopGenres(schedule) {
    var sortToGenres = schedule
        .slice()
        .sort(function (b, a) { return a._embedded.show.genres.length - b._embedded.show.genres.length; });
    var result = sortToGenres.filter(function (item, index, array) {
        return (item._embedded.show.genres.length >=
            sortToGenres[0]._embedded.show.genres.length);
    });
    return result.map(function (el) { return el.name; });
}
console.log(getTopGenres(schedule));
function DataPrim(schedule) {
    var sorted = schedule.sort(function (a, b) {
        return new Date(a._embedded.show.premiered).getTime() -
            new Date(b._embedded.show.premiered).getTime();
    });
    var days = ["ВС", "ПН", "ВТ", "СР", "ЧТ", "ПТ", "СБ"];
    var day = days[new Date(sorted[0]._embedded.show.premiered).getUTCDay()];
